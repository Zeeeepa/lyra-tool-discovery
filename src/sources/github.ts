...
export class GitHubSource {
  ...
  /**
   * Search GitHub for MCP servers matching the given search terms.
  ...
  async searchMCPServers(
    ...
    for (const term of searchTerms) {
      queries.push(`mcp ${term} in:name,description,readme ${dateFilter}`);
    }

    // Also search by topic combinations
    const topicTerms = searchTerms.slice(0, 5); // limit topic queries
    for (const term of topicTerms) {
      queries.push(`topic:mcp topic:${term} ${dateFilter}`);
    }

    // Generic mcp-server name search with category terms
    for (const term of searchTerms.slice(0, 5)) {
      queries.push(`mcp-server ${term} in:name ${dateFilter}`);
    }
